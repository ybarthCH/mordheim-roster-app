...
- **Source primaire, à essayer en premier : le dépôt GitHub `ybarthCH/Musterheim-pdf-warband-ref`.** Public, cloné à la demande (`git clone --depth 1 https://github.com/ybarthCH/Musterheim-pdf-warband-ref.git <destination-hors-du-dépôt-app, ex: /tmp/pdf-ref>` — jamais à l'intérieur de l'arbre de travail de `mordheim-roster-app`, ce clone n'a rien à voir avec ce dépôt applicatif). Contrairement aux uploads de session, il persiste d'une session à l'autre. Structure : dossier `FR/` et dossier `ENG/`, chacun contenant les PDF ainsi qu'un sous-dossier `Markitdown/` (conversion via l'outil `microsoft/markitdown`) avec un `.md` de même nom pour chaque PDF — un peu moins de tokens à lire que le PDF correspondant. **Le dossier FR est incomplet** — plusieurs bandes n'existent qu'en anglais dans `ENG/`, ne suppose jamais qu'un équivalent français existe avant de l'avoir vérifié.
  - **Privilégie le `.md` correspondant plutôt que le PDF** pour une première lecture (moins de tokens), mais reste vigilant : la conversion Markitdown d'un PDF à mise en page multi-colonnes **entrelace l'ordre des lignes** entre les colonnes (le texte de la colonne de droite peut s'intercaler ligne par ligne dans celui de la colonne de gauche) et casse certains tableaux en fragments illisibles. Le contenu textuel lui-même reste fidèle (aucun mot inventé/omis, juste réordonné), mais un passage qui semble incohérent ou dont l'ordre logique paraît cassé n'est PAS une preuve d'erreur dans la règle — relis le PDF original (`pdftotext` ou lecture directe) pour cette section précise avant de conclure quoi que ce soit à partir d'un `.md` dont l'ordre semble suspect.
