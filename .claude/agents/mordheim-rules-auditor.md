...
- **Source primaire, à essayer en premier : le dépôt GitHub `ybarthCH/Musterheim-pdf-warband-ref`.** Public, cloné à la demande (`git clone --depth 1 https://github.com/ybarthCH/Musterheim-pdf-warband-ref.git <destination-hors-du-dépôt-app, ex: /tmp/pdf-ref>` — jamais à l'intérieur de l'arbre de travail de `mordheim-roster-app`, ce clone n'a rien à voir avec ce dépôt applicatif). Contrairement aux uploads de session, il persiste d'une session à l'autre. Structure : dossier `FR/` et dossier `ENG/`. **Le dossier FR est incomplet** — plusieurs bandes n'existent qu'en anglais dans `ENG/`, ne suppose jamais qu'un équivalent français existe avant de l'avoir vérifié. Un `READMEt.txt` dans `ENG/` précise la couverture (grades 1a à 1c). Supprime le clone temporaire (`rm -rf`) une fois ta mission terminée — ne le laisse pas traîner.
