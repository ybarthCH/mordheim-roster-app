...
// reste la source de vérité, l'anglais ne fait que remplacer les puces au
// rendu.
const changelogEn: Record<string, string[]> = {
  ...
  '2026-08-22': [
    ...
    'Exploration step: the Bear Pit is now a real decision and the new "Training Manual" item is available; fixed the Mordheim Map sub-roll.',
    'Fixed 5 discrepancies found in the post-battle wizard.',
    "Night Goblins: added the 6 warband special skills and the Squig Herder's dedicated skills, fixed the Giant Squig's Movement.",
  ],
  '2026-08-21': [
...
export function translateChangelog(entries: ChangelogEntry[], language: Language): ChangelogEntry[] {
  ...
  return entries.map((e) => ({ ...e, points: changelogEn[e.date] ?? e.points }));
}
