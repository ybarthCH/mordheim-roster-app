...
`elfes_noirs`, `expedition_runique`, `orques_noirs`, `amazones_mordheim`, `amazones_lustrie`, `bandits_du_hochland`, `escorteurs_imperiaux`, `hommes_lezards`, `chevaliers_bretonniens`, `tileens`, `hors_la_loi_de_stirwood`, `skavens_pestilens`, `norses`, `pirates`, `guerriers_fantomes`, `maraudeurs_du_chaos`, `gladiateurs`, `gardiens_de_chapelle_bretonniens`, `pilleurs_de_tombes_arabes`, `gobelins_de_la_nuit`, `artilleurs_de_nuln`, `morts_sans_repos`, `cult_of_the_possessed`, `culte_des_tueurs`, `caravanes_marchandes`, `nains_du_chaos`, `marienburgers`.
...
L'essentiel du travail « à l'aveugle » est fait — il ne reste plus de bande jamais auditée, et la repasse ciblée du 1er septembre a fait passer 8 bandes supplémentaires en Niveau 3 (26 bandes au total désormais). Pistes si on veut pousser la confiance plus loin :
...
2. Deux questions de règles restent à trancher par Yannick suite à la repasse du 1er septembre (gabarit dans `PROJECT_DECISIONS.md`, pas encore d'entrée ajoutée) : le Tromblon Nain du Chaos (40 CO, règles propres) est une substitution déjà délibérée dans le code mais dont la source citée ("FH") reste introuvable, alors que les seules sources disponibles (Nains du Chaos [GLM].pdf, Black Dwarfs.pdf) ne décrivent qu'un Tromblon générique à 30 CO ; et l'accès du Franc-Tireur Sorcière pour les Nains du Chaos (probable par analogie avec Maraudeurs du Chaos, jamais nommé explicitement dans une source).
